...
async def test_cleanup_closes_owned_http_client(monkeypatch, tmp_path):
    ...
    pipeline teardown."""
    await assert_cleanup_closes_owned_client(_stt(monkeypatch, tmp_path))
...
async def test_cleanup_still_closes_client_when_super_cleanup_raises(monkeypatch, tmp_path):
    ...
    await assert_cleanup_still_closes_client_when_super_cleanup_raises(
        _stt(monkeypatch, tmp_path), STTService, monkeypatch
    )
...
async def test_transcribe_falls_back_to_top_level_text_when_no_combined_phrases(monkeypatch, tmp_path):
    async with with_mock_transport_client(
        _stt(monkeypatch, tmp_path), lambda r: httpx.Response(200, json={"text": "fallback text"})
    ) as stt:
...
async def test_transcribe_returns_empty_string_when_response_has_neither_field(monkeypatch, tmp_path):
    async with with_mock_transport_client(_stt(monkeypatch, tmp_path), lambda r: httpx.Response(200, json={})) as stt:
...
async def test_transcribe_raises_on_http_error_status(monkeypatch, tmp_path):
    async with with_mock_transport_client(
        _stt(monkeypatch, tmp_path), lambda r: httpx.Response(401, content=b"denied")
    ) as stt:
