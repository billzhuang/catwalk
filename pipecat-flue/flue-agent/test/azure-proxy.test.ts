...
test('recordAndAnnotateUsage + cacheRate accumulate correctly across calls', () => {
  resetMetrics();
  const span = fakeSpan();
  ...
  assert.deepEqual(span.calls, [
    { 'llm.usage.prompt_tokens': 1000, 'llm.usage.completion_tokens': 10, 'llm.usage.cached_tokens': 0 },
    { 'llm.usage.prompt_tokens': 1000, 'llm.usage.completion_tokens': 10, 'llm.usage.cached_tokens': 900 },
  ]);
});
...
test('recordAndAnnotateUsage: missing fields default to 0 instead of poisoning metrics/span with NaN', () => {
  resetMetrics();
  const span = fakeSpan();
